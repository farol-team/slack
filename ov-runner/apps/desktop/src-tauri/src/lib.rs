//! OV Runner desktop: Tauri shell around runner-core.
//! Tray-first UX: the app lives in the tray, window shows status/config.

use runner_core::cloud::run_connection_loop;
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            // Tray: Show / Quit
            let show = MenuItem::with_id(app, "show", "Open OV Runner", true, None::<&str>)?;
        ...
        .expect("error while running ov-runner");
}
