...
- **Key cloud files** (`cloud/app/`): `main.py` (FastAPI: `/runner/v1` WS, `/slack/events`, `/internal/import/*` behind `x-internal-secret`, `/healthz`), `slack_app.py` (mentions, Approve/Deny/Stop buttons, IngestionBuffer), `task_router.py`, `memory.py` (OpenViking HTTP client).
