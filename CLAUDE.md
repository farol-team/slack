...
- **Key runner files** (`runner/crates/opentag-core/src/`): `protocol.rs`, `cloud.rs` (WS loop with reconnect/backoff), `session.rs` (SessionManager: turn policy, memory MCP, permission relay), `config.rs` (config + OS keychain for token), `workspace.rs` (`~/OpenTag/<workspace>/<channel>` derivation), `connect.rs` (browser handoff).
- **Speaking ACP is not in this repo**: the client, the adapter catalogue and the PATH/process-group handling come from [`farol-team/acp-agents`](https://github.com/farol-team/acp-agents) (`acp-client` + `acp-agents`, pinned by tag), shared with WorkRoom and gilb. A protocol-level bug belongs there, with its test; `session.rs` keeps only what is OpenTag's — the cwd allowlist, `auto_allowed`, team memory, and how a turn renders into a thread.
